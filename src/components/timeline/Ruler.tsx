...
export function Ruler({ duration, zoom }: { duration: number, zoom: number }) {
    ...
    useEffect(() => {
        ...
        // Draw
        ctx.fillStyle = '#0F141D'; // bg-panel
        ctx.fillRect(0, 0, width, 24);

        ctx.strokeStyle = '#1C2533'; // border-border
        ctx.beginPath();
        ctx.moveTo(0, 24);
        ctx.lineTo(width, 24);
        ctx.stroke();
        ...
        for (let t = 0; t <= duration + 5000; t += interval) {
            ...
            ctx.strokeStyle = isMajor ? '#334155' : '#1C2533';
            ctx.stroke();
    ...
    return <canvas ref={canvasRef} className="sticky top-0 z-20 block bg-panel h-7 border-b border-border" />;
}
